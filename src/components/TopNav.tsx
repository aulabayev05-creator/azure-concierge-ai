...
export function TopNav({ view, setView }: { view: "guest" | "ops"; setView: (v: "guest" | "ops") => void }) {
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex bg-white border border-[var(--line)] rounded-full p-0.5 text-xs">
            <button onClick={() => setView("guest")} className={`px-3 py-1 rounded-full transition ${view === "guest" ? "bg-[var(--navy)] text-cream" : ""}`}>Гость</button>
            <button onClick={() => setView("ops")} className={`px-3 py-1 rounded-full transition ${view === "ops" ? "bg-[var(--navy)] text-cream" : ""}`}>Операции</button>
          </div>
